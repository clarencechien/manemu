...
- **不用登入就能看**:登入頁「看看介面 →」離線預覽——真實引擎輸出的示範對話,
  但結構上碰不到翻譯引擎(獨立函式 + 伺服器端 401,兩層,見 `app/README.md`)
- 管理:`/admin`(僅 `ADMIN_EMAILS`)——等候名單一鍵核准、設額度級別或自訂秒數
- 引擎:Gemini Live(`gemini-3.1-flash-live-preview` + 語域鎖定口譯 systemInstruction)
- 語言:中 →(日 / 英 / 韓 / 越 / 泰)→ 中
- 狀態:**真機端到端跑通**——實測放開按鈕後 **0.4 秒接話**(harness 預測 ~1.0s,真機更快);
  **可安裝 PWA**(免商店,Android 有安裝按鈕、iOS 加入主畫面);前端版號 **v18**(顯示在頂列)
...
**為什麼是「按住說話」不是連續開麥**(`docs/adr.md` ADR-002):翻譯內核不受影響,
但連續開麥會讓輸入變髒——譯音被自己麥克風收回的回音自噬(失控而非降級)、翻譯方向
只能用語言辨識猜(旅遊場景常混語)、吵雜環境切不出句子邊界。而且**死氣會變長不是變短**
(PTT 的放開手是零誤差的結束信號)。連續對話、低死氣、高品質三選二。
...
**功能一覽**:對談/面對面兩種版面 · 五語下拉(泰語附 ครับ/ค่ะ 尾詞設定) · 場景切換(通用/緊急)+
句型引導 · 🔍 核對意思(回譯確認「對方聽到什麼」) · 🕘 本地翻譯紀錄(含譯音重放) ·
📒 行程術語表 · 點用量開明細面板(剩餘/級別/重置) · 🧪 測試模式(R1 語料收集) ·
登入前介面預覽 · 🛠 admin 入口(僅管理者,admin 頁含每人今日用量)。
...
**未驗**:面對面模式 iOS 真機複驗(v13 修正後)、真機回音場景(外放+吵雜)、
fast 模式在 relay 下的行為(UI 已暫時下架,relay 端仍支援)。
...
**測試裝置**(推送前跑;都在 scratchpad,細節見 `app/README.md`):

| 測試 | 驗什麼 |
| --- | --- |
| `worker-test.mjs` | Worker 路由 × 身分 20 案例(mock env):401/403 閘門、admin 寫入、cron 清除、client-log 開關 |
| `test.mjs` | Playwright 主流程:PTT 一句到底、重播不卡死、語言下拉/泰語尾詞、本地紀錄持久與刪除 |
| `ios-test.mjs` | iOS 五情境模擬:ctx 凍結 fallback、mic keep-alive、假 cancel 瞬放、保險絲、面對面模式 |
| `preview-test.mjs` | 登入前預覽的安全斷言:攔截全部網路 + 監控 getUserMedia → 零 /ws、零 /api/*、零麥克風 |
| `app/scripts/live-check.sh` | **部署後**對正式站的煙霧複驗(資產/版本/安全防線/PWA/headers) |
